...
export default async function StudyAskPage({ params, searchParams }: Props) {
  ...
  const milestone   = typeof sp.milestone   === "string" ? sp.milestone.trim()   : null;
  const milestoneId = typeof sp.milestoneId === "string" ? sp.milestoneId.trim() : null;
  ...
  // When arriving from a milestone card, scope the chat topic tightly
  const chatTopic = milestone
    ? `${g.topic}: ${milestone}`
    : g.topic;
  ...
  return (
    <div className="relative flex h-screen flex-col bg-[#0A0F1E] overflow-hidden">
      ...
      {/* Milestone focus strip — shown only when arriving from a card */}
      {milestone && (
      ...
      <div className="relative flex flex-1 flex-col min-h-0">
        <ChatWindow topic={chatTopic} goalId={goalId} milestoneId={milestoneId ?? undefined} />
      </div>
